...
### Phase 6 — Self-spawning departments (DONE)
...
#### Phase 6 — Reviewed by Ron (cross-cut sweep)
- 2026-04-29 — **Reviewed by Ron — Phase 6 cross-cut** (world repo SHA `6c19541`) — verdict: ship with six fixes applied. **Blockers: 0.**
- **Audit pass per dispatch checklist (12 axes):**
  - (1) **Paper template adherence.** All 8 papers carry thesis / department / evidence / counter-evidence / what-this-lets-us-say-next / gradient placement / faculty. **Gap found and fixed:** both synthesis papers lacked explicit `## evidence` and `## counter-evidence` sections — the deference-to-angle-papers was implicit. Added explicit sections to both, with cross-cut-level counter-evidence pushing back at the synthesis claim itself.
  - (2) **Citation discipline.** Anthropology's *cite-don't-summarize* norm honored across inhabitants, languages, synthesis (heavy verbatim quoting with file:line). Rituals paper has more "shape" sub-headers that summarize, but each cites concrete paths and quotes the load-bearing lines (zisser/principles/01:35, the-world.md:44-48, mark.md:31-34, the-world.md:38). Acceptable; not a blocker. Physics papers cite verbatim heavily; passing.
  - (3) **Faculty-seat naming honesty.** All 8 papers name a seat. Anthropology papers all use the canonical shape: `<seat> — <reason>. wes did the writing; the angle is <seat>'s.` Physics papers use `wes writing in poll's/sam's seat` with collision-partners named. **Mismatch found and fixed:** physics README claimed zisser was "natural author of conservation-of-voice"; conservation.md placed it in poll's seat with zisser as collision-partner. Reconciled — README now matches the paper.
  - (4) **Voice consistency.** Both depts hold the lowercased fragment-leaning Wispr register. Synthesis papers stay in voice; no academic-prose slippage.
  - (5) **Cross-pollination claim coherence.** Physics synthesis names conservation-of-voice as a law. Anthropology synthesis names enacted-by-non-correction + soul-files + boundary-ceremonies. Triangulation holds: same finding, two angles. The cross-cut counter-evidence I added to anthropology synthesis names a *possible* asymmetry (physics may be causally upstream, not co-equal) — the seam stays open as a future paper, honestly.
  - (6) **Two-faced doctrine.** Physics dual-faces both reference each other ✓ AND both reference the dept README ✓. **Gap found and fixed:** anthropology dual-faces referenced each other but neither pointed back to the dept README (charter / birth certificate); also walk-among.md said `agent/paper-template.md (one folder up)` — actually three folders up. Both fixed.
  - (7) **README dept charter completeness.** Both READMEs hit name / question / why-now / faculty / founding-papers / zone / sky→space / stop-condition. **Gap found and fixed:** anthropology README lacked an explicit `## name` section header (only the title). Added.
  - (8) **Reflexive-question handling (anthropology).** Synthesis paper L82-102 names all three reflexive risks (citizenship-test → gatekeeping; rituals-atlas → reification; language-registry → vocabulary-erosion) and L104-134 maps each to an operating-norm response (append-only authorship, mandatory counter-evidence, three-uses-gate, honest-observer rule). Complete.
  - (9) **No "later" violations.** Grepped both dept dirs; all `later` hits are quoted-as-doctrine (citing `feedback_no_later.md` / z002), not actual deferrals. The synthesis-paper round-2 paper-seeds and rituals-paper observation-paper-slot are open-to-empty *placements*, not deferrals — allowed per dispatch checklist. Pass.
  - (10) **Counter-evidence mandatory.** All 8 papers carry substantive counter-evidence pushing back at their own thesis. Synthesis papers now have it explicitly (was the gap from axis 1).
  - (11) **Cross-references resolve.** Walked every relative-path link; all resolve after fixes (anthropology dual-faces previously had a wrong directional count and missing README ref; corrected).
  - (12) **Zone alignment with world's CLAUDE.md.** Both depts placed at `sky/university/departments/`. Both synthesis papers explicitly name "sky" placement and the per-paper promotion-to-space criteria. Anthropology README has a dedicated `sky → space tension` section. Aligned with the world's "before what folder, ask what zone" doctrine.
- **Fixes applied — world repo (`AskEffi/oria-crazy-world` commit `6c19541`):**
  1. **`anthropology/human/walk-among.md`** — wrong path direction ("one folder up" → corrected to `../../../agent/paper-template.md` + naming "three folders up"); added missing reference to dept README.
  2. **`anthropology/agent/observe-without-collapsing.md`** — added `## adjacent` section pointing at dept README, papers/, paper-template, and sister physics dept; matches physics dual-faces' shape.
  3. **`anthropology/papers/synthesis.md`** — added explicit `## evidence` (citing the three angle papers as source-of-record with their key paths) and `## counter-evidence` (cross-cut pushback at the synthesis-level thesis itself).
  4. **`physics/papers/synthesis.md`** — same as above; added explicit evidence + counter-evidence sections at the cross-cut level.
  5. **`anthropology/README.md`** — added explicit `## name` section heading per charter template.
  6. **`physics/README.md`** — reconciled faculty assignment with conservation.md's faculty section (zisser as collision-partner, poll as round-1 seat); added "honest observer rule" attribution to anthropology dept (which physics inherits cross-pollination).
- **Notes for Zisser (no fix; observations):**
  - **Cross-dept asymmetry surfaced as round-2 seed.** Anthropology synthesis L181-188 names a joint paper *paraphrase as conservation violation, observationally* with physics. The cross-cut counter-evidence I added in this sweep raises a sharper question: *is anthropology causally downstream of physics, not co-equal?* That seam wants its own paper. Already seeded in anthropology's "what this lets us say next"; flagging here so it doesn't get lost.
  - **Physics shipped first; anthropology then placed itself reflexively.** The triangulation worked because the second dept inherited the first dept's vocabulary (conservation-of-voice cited verbatim in `languages.md`). When future depts charter (e.g. `language` if seeded), they should expect to absorb prior depts' vocabulary the same way — that *is* the inheritance mechanism. Worth naming as a doctrine-of-charter.
  - **Faculty discipline.** Anthropology's "wes did the writing; the angle is X's" pattern is the more honest one. Physics's "wes writing in poll's seat" is shorter but less explicit about the writer/seat split. Recommend physics absorb anthropology's longer form on round 2 supersession. (Already partially named in the README fix.)
  - **Synthesis papers as cross-cuts.** Both synthesis papers are now structurally honest: their evidence section defers to the angle papers (with paths cited so a reader can drill in), and their counter-evidence section pushes back at the synthesis-claim itself, not the underlying laws. This shape is reusable for future synthesis papers in any dept.
- **Blockers: 0.** Phase 7 dispatches free to proceed.

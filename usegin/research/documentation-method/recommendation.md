...
- **A. Promote the skill-lab `purpose.md` + `retro-guide.md` pattern
  out of `.claude/skill-lab/` into a sub-app at `usegin/lab/<topic>/`** —
...
- **B. Make `usegin/zettel/RD/<topic>/whiteboard.md` the canonical form
...
- **C. Build a new `usegin/log/` sub-app** — append-mostly per-session
  log entries (`usegin/log/2026-04-27-documentation-method.md`) that
...
- Generalising it from "per skill" to "per Gin-internal initiative"
  ...
  `usegin/lab/`. That fits z021 (Gin is the umbrella) without needing
  any new convention.
...
- Cross-cuts cleanly with what we already have: zettels stay atomic
  (one thought, two sides), `usegin/lab/<topic>/` is the *workshop* for
  ideas that take more than one zettel to develop, Linear stays for
  shipping. Three layers, clear addressing.

**Price:**
- New top-level folder `usegin/lab/`. One more place to navigate (z001
...
- Generalising "skill-lab" semantics may dilute the original meaning
  ...
  topics in `usegin/lab/` won't have sessions in that sense, so the
  retro-guide column may stay empty for them.
- Open-to-empty (z003) handles this — a `usegin/lab/<topic>/` can ship
...
- The skill-lab pattern *succeeds because skills are auto-triggered*.
  ...
  `usegin/lab/<topic>/purpose.md` only gets read when someone navigates
  there. Mitigation: make the sub-app's READMEs (per z010) point into
  `usegin/lab/`, so any Gin orienting in the parent sub-app finds the
  lab folder by routine. Also: if a `usegin/lab/<topic>/` matures into a
  ...
  R&D.
- We commit to a third name (`usegin/zettel/`, `usegin/consultant/`,
  `usegin/lab/`). Per z021 the umbrella is supposed to absorb sub-apps
  cleanly; one more sub-app is cheap, but the *meta-rule* — "if
  unsure where it goes, here's the sub-app for that" — needs to be
  stated explicitly in `usegin/README.md` or it will erode.

**For you to weigh:**

1. **UX for capturing a thought mid-session.** Zettels are ~30s
   capture (one file, append). A `usegin/lab/<topic>/` is heavier — you
...
2. **Cross-team coordination.** Right now Lihu is the only zettel
   ...
   `usegin/lab/<topic>/` to find current state of a Gin-internal effort?
   `usegin/lab/` is more discoverable than zettel front-matter
   threading, but discoverability stops mattering if nobody has the
   habit of opening it. Worth weighing: do they prefer the `usegin/lab/`
...
3. **Scope drift into "everything-bin".** The biggest risk in
   ...
   `usegin/lab/` becomes the new `docs/`, we've moved the graveyard, not
...
4. **Longevity vs the system you're already building.** ENG-5381's
   ...
   `usegin/lab/` weakens — you might just want richer multi-zettel
...
- New folder: `usegin/lab/`. Sibling to `usegin/zettel/`, `usegin/consultant/`,
  `usegin/research/`.
- One `usegin/lab/README.md` stating: *"Cross-session Gin-internal work
...
- First seed: this very research moves from
  `usegin/research/documentation-method/` into
  `usegin/lab/documentation-method/` once the dilemma is resolved — and
  the resolution is the first Changelog row.
- `things-we-grow.md` gets a row.
- One zettel — `z028-usegin (umbrella)-as-workshop` or similar — to record the

...
- **Action form chosen in real time.** Don't pre-bake a policy ("always leave as draft" or "always send"). Judge per situation, propose, let the user steer. **Connector capabilities differ — know them, don't guess:**
  - **Gmail (`team-gmail`)** — `create_draft` only via the connector (no direct send). Per-user Apps Script auto-send is available: prefix the body with `[auto-send]\n\n` and the user's script picks it up within ~1 min, strips line 1, and sends. **Every Gmail draft must end with `Sent from <FirstName>'s Claude Code`** as its last body line — mandatory honest disclosure, applies to manual-send and auto-send drafts alike. **Only use the auto-send marker when the user explicitly asks** ("send it", "auto-send"); otherwise default to a plain draft. Probe once per session whether the user has Apps Script wired up — if not, the marker is harmless but the draft sits in Drafts. **After queuing an auto-send, verify** — wait ~90s, then `get_thread` on the target thread and confirm: (a) exactly one new sent message with the expected body, (b) the `[auto-send]` marker was stripped (no leakage), (c) the disclosure footer is present, (d) no duplicate previous-draft also went out. Report the verification result back to the user; if anything is off, surface it immediately and propose a recovery — don't claim "sent" until verified.

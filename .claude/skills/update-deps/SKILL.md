...
- **`google-genai` (in python-services) — stuck at 1.x (transitive). Latest `2.0.1`.** `google-cloud-aiplatform>=1.149.0` requires `google-genai>=1.66.0,<2.0.0`. Resolver-confirmed 2026-05-09: forcing `google-genai>=2.0.0` rejects with "google-cloud-aiplatform>=1.149.0 cannot be used." Note: `tools/google-file-search` (a standalone POC) has no aiplatform dep so it CAN take 2.0 — its own bump tracked separately.

...
- **`websockets` — stuck at 15.x (transitive). Latest `16.0`.** `realtime==2.30.0` requires `websockets>=11,<16`; `supabase==2.30.0` pins `realtime==2.30.0`. Re-probed 2026-05-09 via `uv lock --upgrade-package 'websockets>=16' --dry-run` — resolver rejects with "supabase==2.30.0 cannot be used" chain. Newer realtime versions exist with `websockets>=16` but supabase doesn't pin them yet. Pure supabase-stable-bump unblock.
...
- **`claude-agent-sdk` — exact pin (`==0.1.80` as of 2026-05-09).** Pin reason isn't documented in the repo; treat each bump as deliberate (we touch a lot of the SDK's surface — streaming, tools, hooks; plus we monkey-patch `_internal.message_parser` in `sdk_patches/error_field.py`). Don't lump into a routine sweep — bump in its own commit with diff review of the SDK between versions and a focused `tests/integration/claude/` pass (covers streaming, tool surface, auth switching, monkey-patched parser via `test_sdk_error_sentry`).

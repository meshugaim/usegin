...
- **Each Python tool is independent.** Tools with their own `pyproject.toml` + `uv.lock` (e.g., `tools/fathom/`) have separate dependency trees. Upgrade each one individually.
...
**`supabase` is the common choke point.** Three of the four remaining transitive caps below (`cachetools`, `rich`, `websockets`) all trace back to `supabase` via its own pinned sub-deps (`storage3`→`pyiceberg`, and `realtime`). 2026-05-19 update: re-verified live via `uv lock --upgrade-package … --dry-run` for websockets/rich/cachetools — all resolver-blocked, no movement. supabase still at 2.30.0; PyPI has `3.0.0a1` pre-release. Re-check after each supabase stable. **google-genai 2.x is now unblocked** as of 2026-05-19 (aiplatform 1.153.x lifted the cap) and bumped this sweep — entry deleted.
...
- **`cachetools` — stuck at 6.x (transitive). Latest `7.1.3`.** `pyiceberg>=0.10.0` requires `cachetools<7` (chain: `supabase` → `storage3` → `pyiceberg`). Verified sole path via `uv tree --package pyiceberg --invert`. Re-confirmed 2026-05-19: supabase 2.30.0 still pulls the same pyiceberg constraint. Unblock: pyiceberg release that moves the cap, or supabase stable that pins newer pyiceberg.
- **`rich` — stuck at 14.x (transitive). Latest `15.0.0`.** `pyiceberg>=0.10.0` requires `rich>=10.11.0,<15.0.0` (supabase chain). `tach@0.35.0` (dev) also bounds `rich<15` via `rich>=13.5.2,<15.0.0` — both caps must lift for rich to move. Re-confirmed 2026-05-19 via dry-run.
- **`websockets` — stuck at 15.x (transitive in python-services). Latest `16.0`.** `realtime==2.30.0` requires `websockets>=11,<16`; `supabase==2.30.0` pins `realtime==2.30.0`. Re-probed 2026-05-19 via `uv lock --upgrade-package 'websockets>=16' --dry-run` — resolver rejects with "supabase==2.30.0 cannot be used" chain. Newer realtime versions exist with `websockets>=16` but supabase doesn't pin them yet. Pure supabase-stable-bump unblock. *Note: `tools/google-file-search/` has no supabase dep and IS at websockets 16.0.*
- **`protobuf` — stuck at 6.x (transitive). Latest `7.34.1`.** `google-cloud-aiplatform>=1.152.0` requires `protobuf>4.21.5,<7.0.0` (re-confirmed 2026-05-19 — aiplatform 1.153.x still pins). **Not supabase** — blocked on Google's release cadence.
- **`pdfminer-six` — stuck at `20251230` (transitive). Latest `20260107`.** `pdfplumber==0.11.9` exact-pins `pdfminer-six==20251230`. PyPI's latest pdfplumber is still 0.11.9 (re-confirmed 2026-05-19 — `uv lock --upgrade-package pdfminer-six --dry-run` returned `No lockfile changes detected`). Unblock requires a new pdfplumber release.
- **`claude-agent-sdk` — exact pin (`==0.2.82` as of 2026-05-19).** Treat each bump as deliberate (we touch a lot of the SDK's surface — streaming, tools, hooks). Don't lump into a routine sweep — bump in its own commit with diff review and a focused `tests/integration/claude/` pass (streaming, tool surface, auth switching). Historical note: prior versions had an `error_field` parsing bug (top-level `error` field on `AssistantMessage` wasn't read); we monkey-patched it in `sdk_patches/error_field.py`. The bug was fixed upstream by 0.1.81, and the patch was deleted in the 0.1.81→0.2.82 bump (2026-05-19). If error-field-style parser bugs reappear, that pattern is the playbook.

### `nextjs-app/`

- **`react` / `react-dom` — exact pin (no caret), both at 19.2.6.** Lockstep siblings; versions must match exactly. Bump as a coordinated pair only.
- **`eslint-plugin-storybook` — exact pin (no caret), currently 10.4.0.** Lockstep with `storybook` and `@storybook/*` (currently 10.4.0). Bump only when all Storybook packages move together — `bun update @storybook/*` won't touch the exact-pinned `eslint-plugin-storybook`, edit by hand.
...
- **`@types/ws` — must be a direct devDep, not transitive.** ws 8.20.0 (2026-05-09 bump) no longer pulls `@types/ws` transitively the way 8.18.x did, so `lib/supabase-client.ts`'s `import ws from "ws"` fails tsgo with TS7016 "Could not find a declaration file for module 'ws'". Add `@types/ws` as a direct devDependency. Do not assume the transitive types come along.

*(No entries yet for `landing-app/` or Python tools. Add as they emerge.)*

...
- **`claude-agent-sdk` — exact pin (`==0.1.71`).** Latest `0.1.73`. Pin reason isn't documented in the repo; treat each bump as deliberate (we touch a lot of the SDK's surface — streaming, tools, hooks). Don't lump into a routine sweep — bump in its own commit with diff review of the SDK between versions and a focused agent-streaming integration-test pass. Re-confirmed 2026-05-05.

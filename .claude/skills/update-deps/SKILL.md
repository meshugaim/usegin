...
- **`websockets` — stuck at 15.x (transitive). Latest `16.0`.** `realtime==2.29.0` requires `websockets>=11,<16`; `supabase==2.29.0` pins `realtime==2.29.0`. Re-probed 2026-05-05 via dry-run resolver — still no movement. Newer realtime versions exist with `websockets>=16` but supabase doesn't pin them yet. Pure supabase-stable-bump unblock.
...
- **`pdfminer-six` — stuck at `20251230` (transitive). Latest `20260107`.** `pdfplumber==0.11.9` exact-pins `pdfminer-six==20251230`. PyPI's latest pdfplumber is still 0.11.9 (re-confirmed 2026-05-05 — `uv lock --upgrade-package pdfplumber --dry-run` returned `No lockfile changes detected`). Unblock requires a new pdfplumber release.
- **`claude-agent-sdk` — exact pin (`==0.1.73` as of 2026-05-05).** Pin reason isn't documented in the repo; treat each bump as deliberate (we touch a lot of the SDK's surface — streaming, tools, hooks; plus we monkey-patch `_internal.message_parser` in `sdk_patches/error_field.py`). Don't lump into a routine sweep — bump in its own commit with diff review of the SDK between versions and a focused `tests/integration/claude/` pass (covers streaming, tool surface, auth switching, monkey-patched parser via `test_sdk_error_sentry`).

...
**`supabase` is the common choke point — but NOT the unblock we hoped for.** Three of the five transitive caps below (`cachetools`, `rich`, `websockets`) all trace back to `supabase` via its own pinned sub-deps (`storage3`→`pyiceberg`, and `realtime`). 2026-05-14 update: re-verified live via `uv lock --upgrade-package … --dry-run` for websockets/rich/google-genai — all three resolver-blocked, no movement. supabase still at 2.30.0; PyPI has `3.0.0a1` pre-release. Re-check after each supabase stable.
...
- **`cachetools` — stuck at 6.x (transitive). Latest `7.1.1`.** `pyiceberg>=0.10.0` requires `cachetools<7` (chain: `supabase` → `storage3` → `pyiceberg`). Verified sole path via `uv tree --package pyiceberg --invert`. Re-confirmed 2026-05-09: supabase 2.30.0 still pulls the same pyiceberg constraint. Unblock: pyiceberg release that moves the cap, or supabase stable that pins newer pyiceberg.
- **`rich` — stuck at 14.x (transitive). Latest `15.0.0`.** `pyiceberg>=0.10.0` requires `rich>=10.11.0,<15.0.0` (supabase chain). `tach@0.35.0` (dev) also bounds `rich<15` via `rich>=13.5.2,<15.0.0` — both caps must lift for rich to move. Re-confirmed 2026-05-14 via dry-run.
- **`websockets` — stuck at 15.x (transitive). Latest `16.0`.** `realtime==2.30.0` requires `websockets>=11,<16`; `supabase==2.30.0` pins `realtime==2.30.0`. Re-probed 2026-05-14 via `uv lock --upgrade-package 'websockets>=16' --dry-run` — resolver rejects with "supabase==2.30.0 cannot be used" chain. Newer realtime versions exist with `websockets>=16` but supabase doesn't pin them yet. Pure supabase-stable-bump unblock.
...
- **`google-genai` — held at 1.75.0 across both `python-services/` and `tools/google-file-search/`. Latest `2.2.0` (2026-05-14).** `python-services` is *blocked*: `google-cloud-aiplatform>=1.152.0` requires `google-genai>=1.75.0,<2.0.0` (resolver-confirmed 2026-05-14 — forcing `>=2.0.0` rejects with "google-cloud-aiplatform>=1.151.0 cannot be used" chain; 1.152 still pins genai<2). `tools/google-file-search/` is *deliberately parked* even though it has no aiplatform dep and could move: kept at 1.x for parity with `python-services` until aiplatform lifts the cap, so we bump everything together. The 2.0 break is "Interactions API only" (SSE event renames, deprecated `response_format`) — `GenerateContent` and file_search are unaffected, so risk on tools/google-file-search is low when we do bump. Revisit when `google-cloud-aiplatform` ships a release allowing `google-genai>=2.0.0`.
- **`pdfminer-six` — stuck at `20251230` (transitive). Latest `20260107`.** `pdfplumber==0.11.9` exact-pins `pdfminer-six==20251230`. PyPI's latest pdfplumber is still 0.11.9 (re-confirmed 2026-05-09 — `uv lock --upgrade-package pdfplumber --dry-run` returned `No lockfile changes detected`). Unblock requires a new pdfplumber release.
- **`claude-agent-sdk` — exact pin (`==0.1.81` as of 2026-05-14).** Pin reason isn't documented in the repo; treat each bump as deliberate (we touch a lot of the SDK's surface — streaming, tools, hooks; plus we monkey-patch `_internal.message_parser` in `sdk_patches/error_field.py`). Don't lump into a routine sweep — bump in its own commit with diff review of the SDK between versions and a focused `tests/integration/claude/` pass (covers streaming, tool surface, auth switching, monkey-patched parser via `test_sdk_error_sentry`). Last bump 0.1.80→0.1.81: upstream notes "Updated bundled Claude CLI to version 2.1.139" — no surface change.
